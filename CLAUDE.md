...
**Selection is never rendered via the real `<textarea>`'s `::selection`.** Bug (found in the sibling port zettelium-web, confirmed here too): with `.heading-sizes` enabled, a taller heading line earlier in the document makes `#ed-highlight`'s line boxes drift a few px from `#ed-input`'s (a `<textarea>` can't render one line taller than another; the overlay's enlarged heading line, even with a compensated `line-height`, still ends up slightly taller in practice due to real font ascent/descent metrics at the larger size). Giving `::selection` a `color` makes the *textarea's own* glyphs visible for the selected range, at the textarea's (undrifted) position — visually offset from the surrounding overlay text. Fix: `#ed-input::selection` is fully invisible (`background/color: transparent`) when `CSS.highlights` is supported; the selection is instead mirrored as a `Highlight` (**CSS Custom Highlight API**, Chromium-only) directly on `#ed-highlight`'s own text nodes, via `Editor.updateSelectionHighlight()` (`editor.js`): `domPositionForOffset()` walks `#ed-highlight`'s text nodes with a `TreeWalker` to convert a raw-text character offset into a DOM `{node, offset}` (their concatenation in document order reconstructs the raw text exactly — `highlight()` only ever wraps substrings in spans, never adds/removes characters within a line), then a `Range` between the two endpoints is registered via `CSS.highlights.set('ed-selection', new window.Highlight(range))` (`window.Highlight` explicitly — not required in this file today, but keeps parity with zettelium-web where a same-named `Highlight` module shadows the constructor). Background+glyph are then painted in the same pass as the rest of the overlay's text — full contrast, no drift, regardless of the residual overlay/textarea height mismatch. Must be re-run after **every** DOM mutation of `#ed-highlight` (`rehighlight()`, `_tryIncrementalRepaint()`, `syncBlockCursor()`/`_patchCursorLine()`, `_applyMarks()` — stale `Range` endpoints on detached nodes fail silently) and on `document`'s `selectionchange` while the textarea is focused; cleared on `blur` and in `Editor.close()`. No-op (native translucent `::selection` fallback via `--bg-sel-rgb`/`.custom-highlight-supported` takes over) on a browser without `CSS.highlights`.
...
Brave disables the File System Access API by default (its own privacy setting — verified by inspecting the installed binary: `window.showDirectoryPicker` is `undefined` out of the box, present with `--enable-features=FileSystemAccessAPI`). The app's own "not available in this browser" message looks identical to the real Firefox/Safari case, but the fix here is just a flag: `brave://flags/#file-system-access-api` → Enabled → relaunch. Documented in README.md's Browser notes.
